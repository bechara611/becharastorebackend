...
export const PostCategorias = async (req = request, res = response) => {
    ...
    try {
     ...
     //TODO comprobar si el category existe
        ...
        const categorias = await Categories.find();
     return   res.status(200).json({ ok: true, msg: ' post Categorias',nueva,categorias })
    } catch (error) {
        console.log(error)
        return res.status(400).json({
            ok: false,
...
export const DeleteCategorias = async (req = request, res = response) => {
    ...
    try {
        ...
        const eliminada = await Categories.findByIdAndDelete(existe._id,{new:true})
        const categorias = await Categories.find();
       return res.status(200).json({ ok: true, msg: ' Delete Categorias', eliminada,categorias })
    } catch (error) {
        console.log(error)
        return res.status(400).json({
            ok: false,
